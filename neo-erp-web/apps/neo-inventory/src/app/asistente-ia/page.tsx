...
export default function AIChatAssistantPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: '👋 ¡Hola! Soy tu **Asistente de Inteligencia Artificial Morpheus** para la gestión de **Inventario**.\n\nPuedo ayudarte a consultar stock actual, analizar movimientos de almacén, valorizar tu inventario y auditar el historial de movimientos de productos.\n\n¿De qué te gustaría hablar hoy?'
    }
  ]);
  ...
  const chips = [
    "¿Cuál es el stock actual de Harina Pan?",
    "Muestra el detalle de inventario por almacén",
    "Ver kardex del producto Harina Pan"
  ];
    ...
    <div className="w-full max-w-[1000px] mx-auto h-[calc(100vh-100px)] py-6 px-4 flex flex-col gap-4">
      <div className="flex-shrink-0">
        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
          <span className="text-2xl">✨</span> Asistente Analítico IA (Inventario)
        </h1>
        <p className="text-slate-500 text-xs font-medium mt-1">Monitorea y analiza tus niveles de stock, valorización e historial de movimientos de inventario.</p>
      </div>
              ...
              <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fadein`}>
                <div className={`max-w-[85%] rounded-3xl p-5 border shadow-sm ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${isUser ? 'text-sky-100' : 'text-sky-600'}`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-slate-50 text-slate-500 border border-slate-100 rounded-3xl rounded-tl-none p-5 max-w-[80%] flex items-center gap-3">
                <i className="pi pi-spin pi-spinner text-sky-600 text-lg"></i>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {chips.map((chip, cIdx) => (
              <button
                ...
                className="bg-white hover:bg-sky-50 text-slate-600 hover:text-sky-600 border border-slate-200 hover:border-sky-300 rounded-full px-4 py-1.5 text-xs font-semibold whitespace-nowrap transition-all duration-200 shadow-sm"
              >
                💡 {chip}
              </button>
            ))}
          ...
          >
            <div className="relative flex-1">
              <InputText
                ...
                placeholder="Pregúntale a la IA sobre stock de productos, almacenes o movimientos..."
                disabled={loading}
                className="w-full py-3 pl-4 pr-12 border-slate-200 rounded-2xl focus:ring-4 focus:ring-sky-500/10 font-medium text-slate-700 bg-white"
              />
            </div>
            <Button
              type="submit"
              ...
              className="!bg-sky-600 hover:!bg-sky-700 border-none px-5 py-3 rounded-2xl shadow-md shadow-sky-100 text-white"
            />
          </form>
        </div>
      </div>
