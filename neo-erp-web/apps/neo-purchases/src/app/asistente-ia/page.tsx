...
export default function AIChatAssistantPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: '👋 ¡Hola! Soy tu **Asistente de Inteligencia Artificial Morpheus** para el departamento de **Compras**.\n\nPuedo ayudarte a buscar órdenes de compra, analizar transacciones recientes de proveedores, calcular volúmenes de adquisición y analizar tus costos de reposición.\n\n¿De qué te gustaría hablar hoy?'
    }
  ]);
  ...
  const chips = [
    ...
    "Analiza los productos provistos por Diageo",
    ...
    <div className="w-full max-w-[1000px] mx-auto h-[calc(100vh-100px)] py-6 px-4 flex flex-col gap-4">
      <div className="flex-shrink-0">
        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
          <span className="text-2xl">✨</span> Asistente Analítico IA (Compras)
        </h1>
        <p className="text-slate-500 text-xs font-medium mt-1">Consulta y analiza proveedores, costos de reposición y órdenes de compra mediante inteligencia artificial.</p>
      </div>
              ...
              <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fadein`}>
                <div className={`max-w-[85%] rounded-3xl p-5 border shadow-sm ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${isUser ? 'text-emerald-100' : 'text-emerald-600'}`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-slate-50 text-slate-500 border border-slate-100 rounded-3xl rounded-tl-none p-5 max-w-[80%] flex items-center gap-3">
                <i className="pi pi-spin pi-spinner text-emerald-600 text-lg"></i>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {chips.map((chip, cIdx) => (
              <button
                ...
                className="bg-white hover:bg-emerald-50 text-slate-600 hover:text-emerald-600 border border-slate-200 hover:border-emerald-300 rounded-full px-4 py-1.5 text-xs font-semibold whitespace-nowrap transition-all duration-200 shadow-sm"
              >
                💡 {chip}
              </button>
            ))}
          ...
          >
            <div className="relative flex-1">
              <InputText
                ...
                placeholder="Pregúntale a la IA sobre proveedores, órdenes de compra o costos..."
                disabled={loading}
                className="w-full py-3 pl-4 pr-12 border-slate-200 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 font-medium text-slate-700 bg-white"
              />
            </div>
            <Button
              type="submit"
              ...
              className="!bg-emerald-600 hover:!bg-emerald-700 border-none px-5 py-3 rounded-2xl shadow-md shadow-emerald-100 text-white"
            />
          </form>
        </div>
      </div>
