...
function AppSidebarContent() {
  ...
  const catalogItems = [
    { label: 'Catálogo de Productos', icon: 'pi pi-box', href: '/products' },
    { label: 'Categorías de Compra', icon: 'pi pi-tags', href: '/categories' },
  ];
          ...
          return (
            <li key={item.href}>
               <Link href={item.href} className={`flex items-center gap-[12px] px-[12px] py-[10px] rounded-lg transition-all duration-300 group relative overflow-hidden ${isActive ? 'bg-[#1e293b]/60 text-white font-medium' : 'text-slate-400 hover:bg-[#1e293b]/40 hover:text-slate-200'}`}>
                 {isActive && (
                  <>
                    <div className="absolute left-0 top-0 h-full w-[4px] bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] z-20"></div>
                    <div className="absolute left-0 top-0 h-full w-[96px] bg-gradient-to-r from-emerald-500/25 to-transparent z-10"></div>
                  </>
                 )}
  ...
  return (
    <div className="w-[256px] h-screen bg-[#0f172a] border-r border-[#1e293b] text-slate-300 flex flex-col transition-all duration-300 z-20 sticky top-0 flex-shrink-0" style={{boxSizing: 'border-box'}}>
      <div className="h-[64px] flex items-center px-[24px] border-b border-[#1e293b] bg-[#0f172a] mb-4">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/30 mr-[12px]">
        ...
        <div className="flex items-baseline gap-[6px] whitespace-nowrap">
          ...
          <span className="font-medium text-emerald-400 text-[13px] tracking-wide">PURCHASES</span>
        </div>
      </div>
      ...
      <div className="p-[16px] border-t border-[#1e293b] bg-[#0f172a]">
        <div className="rounded-xl p-[8px] flex items-center gap-[12px] bg-[#1e293b]/50 border border-[#1e293b] hover:bg-[#1e293b] transition-colors cursor-pointer">
          <div className="w-[36px] h-[36px] rounded-full bg-emerald-950 border border-emerald-500/50 flex items-center justify-center text-emerald-300 font-bold text-[14px] shadow-inner shadow-emerald-500/20">
            {userInitials}
          </div>
...
export function AppSidebar() {
  return (
    <Suspense fallback={<div className="w-[256px] h-screen bg-[#0f172a] border-r border-[#1e293b] flex-shrink-0" />}>
      <AppSidebarContent />
    </Suspense>
  );
}
