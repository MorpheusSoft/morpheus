...
export default function NewPricingSessionPage() {
  ...
  const updateTypes = [
    { label: 'Actualización de Costos (COST)', value: 'COST' },
    { label: 'Actualización de Precios (PRICE)', value: 'PRICE' },
    { label: 'Ambos (Costos y Precios) (BOTH)', value: 'BOTH' }
  ];
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-12">
       <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100">
          <div className="mb-8">
             <div className="inline-flex w-12 h-12 bg-rose-50 text-rose-600 rounded-2xl items-center justify-center mb-4">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
             <div>
                ...
                <InputText 
                   ...
                   className="w-full p-3 bg-slate-50 border-slate-200 rounded-xl focus:ring-4 focus:ring-rose-500/20 transition-all font-medium" 
                />
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                     <label className="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Tipo de Actualización</label>
                     <Dropdown 
                        value={form.update_type} 
                        options={updateTypes} 
                        onChange={(e) => setForm({...form, update_type: e.value})} 
                        className="w-full border-slate-200 bg-slate-50 rounded-xl" 
                     />
                  </div>
             </div>

             {loading && <ProgressBar mode="indeterminate" style={{ height: '6px' }} color="#e11d48" className="mt-4" />}

             <div className="flex justify-end gap-3 mt-8">
                <Button label="Cancelar" type="button" text severity="secondary" onClick={() => router.push('/')} className="px-6 rounded-xl font-bold" />
