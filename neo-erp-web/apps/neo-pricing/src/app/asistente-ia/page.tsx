...
export default function AIChatAssistantPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: '👋 ¡Hola! Soy tu **Asistente de Inteligencia Artificial Morpheus**.\n\nEstoy aquí para ayudarte a analizar costos, simular márgenes, auditar variaciones de precios y explorar los datos financieros de tus sucursales.\n\n¿De qué te gustaría hablar hoy?'
    }
  ]);
  ...
  const chips = [
    "Analiza el margen de Harina Pan en Sucursal Principal",
    "¿Cuáles son los productos con margen bruto menor al 25%?",
    "Compara el costo estándar con costo de reposición de Harina Pan"
  ];
  ...
  // Custom Lightweight Markdown-to-HTML parser inside React
    ...
    <div className="w-full max-w-[1000px] mx-auto h-[calc(100vh-100px)] py-6 px-4 flex flex-col gap-4">
      ...
      <div className="flex-shrink-0">
        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
          <span className="text-2xl">✨</span> Asistente Analítico IA (Costos & Precios)
        </h1>
        <p className="text-slate-500 text-xs font-medium mt-1">Consulta estadísticas de ganancias, simula aumentos y audita la base de datos con lenguaje natural.</p>
      </div>
            ...
            return (
              <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fadein`}>
                <div className={`max-w-[85%] rounded-3xl p-5 border shadow-sm ${
                  isUser 
                    ? 'bg-rose-500 text-white border-rose-600 rounded-tr-none' 
                    : 'bg-slate-50 text-slate-700 border-slate-100 rounded-tl-none'
                }`}>
                  {/* Avatar / Name indicator */}
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${isUser ? 'text-rose-100' : 'text-rose-500'}`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-slate-50 text-slate-500 border border-slate-100 rounded-3xl rounded-tl-none p-5 max-w-[80%] flex items-center gap-3">
                <i className="pi pi-spin pi-spinner text-rose-500 text-lg"></i>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {chips.map((chip, cIdx) => (
              <button
                ...
                className="bg-white hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-300 rounded-full px-4 py-1.5 text-xs font-semibold whitespace-nowrap transition-all duration-200 shadow-sm"
              >
                💡 {chip}
              </button>
            ))}
          ...
          >
            <div className="relative flex-1">
              <InputText
                ...
                placeholder="Pregúntale a la IA sobre costos, precios, compras o stock..."
                disabled={loading}
                className="w-full py-3 pl-4 pr-12 border-slate-200 rounded-2xl focus:ring-4 focus:ring-rose-500/10 font-medium text-slate-700 bg-white"
              />
            </div>
            <Button
              type="submit"
              ...
              className="!bg-rose-500 hover:!bg-rose-600 border-none px-5 py-3 rounded-2xl shadow-md shadow-rose-100 text-white"
            />
          </form>
        </div>
      </div>
