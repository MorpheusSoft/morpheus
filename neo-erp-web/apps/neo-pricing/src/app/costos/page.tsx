...
export default function CostSessionsListPage() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      // Filter for Cost-related updates
      const costSessions = (data || []).filter((s: any) => s.update_type === 'COST' || s.update_type === 'BOTH');
      setSessions(costSessions);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center gap-3">
            <span className="text-slate-400 font-normal">📉</span> Actualizar Costos
          </h1>
          <p className="text-slate-500 mt-1 font-medium">Controla y audita los costos de adquisición y reposición de tus productos.</p>
        </div>
        <Button 
          label="Nueva Actualización de Costo" 
          icon="pi pi-plus" 
          ...
          onClick={() => router.push('/costos/new')} 
        />
      </div>

      <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/30 border border-slate-100 overflow-hidden">
        <DataTable value={sessions} loading={loading} emptyMessage="No hay sesiones de actualización de costos registradas" paginator rows={10} className="p-4">
          ...
          <Column field="target_cost_type" header="TIPO DE COSTO" body={(r: any) => (
            <span className={`text-xs font-semibold px-2 py-0.5 rounded ${r.target_cost_type === 'REPLACEMENT' ? 'text-violet-600 bg-violet-50 border border-violet-100' : 'text-amber-600 bg-amber-50 border border-amber-100'}`}>
              {r.target_cost_type === 'REPLACEMENT' ? 'REPOSICIÓN' : 'ESTÁNDAR'}
            </span>
          )}></Column>
            ...
            <div className="flex gap-2 justify-end">
              {r.status === 'DRAFT' && (
                <Button icon="pi pi-trash" rounded text severity="danger" onClick={async () => {
                  if(confirm('¿Seguro quieres eliminar este borrador de costo?')) {
