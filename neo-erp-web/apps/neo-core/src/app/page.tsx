...
export default function AppSwitcher() {
  const modules = [
    ...
    },
    {
      id: "hr",
      title: "Neo Nómina",
      description: "Acumuladores, recibos de pago y parámetros legales.",
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center p-6 sm:p-12 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {modules.map((mod) => (
            <div key={mod.id} className="relative group">
              ...
              <Link 
                href={mod.path}
                className={`flex flex-col h-full relative glass-card p-8 rounded-[2rem] overflow-hidden ${mod.active ? 'cursor-pointer' : 'cursor-not-allowed grayscale'}`}
                onClick={(e) => {
                  if (!mod.active) {
                    e.preventDefault();
                    alert('Módulo en desarrollo (Fase 3)');
                  }
                }}
              >
                {/* Status Badge */}
