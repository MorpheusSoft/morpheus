...
public class SalesExtractorWorker : BackgroundService
{
    ...
    private async Task<(bool success, int count)> ProcessSalesBatchAsync(DirectExtractorConfig config, DateTime lastSync, DateTime upperBound, CancellationToken stoppingToken)
    {
        ...
        string query = @"
            select @FacilityId as facility_id, @FacilityCode as facility_code, @DepositCode as deposit_code, c_Numero, f_Fecha + convert(time,h_Hora) as f_Fecha, Cod_Principal, Cantidad, Precio, Subtotal, Impuesto, Total  
        ...
        try
        {
            ...
            var sales = (await connection.QueryAsync(query, new { FacilityId = facilityId, FacilityCode = facilityCode, DepositCode = depositCode, LastSync = lastSync, UpperBound = upperBound }, commandTimeout: 180)).ToList();
