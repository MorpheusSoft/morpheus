...
try:
    ...
    with urllib.request.urlopen(req, timeout=120) as response:
        ...
        print("Raw response from Gemini received.")
...
if parsed_items:
    # Try local database reconciliation
    ...
    try:
        ...
        if not supplier:
            print("No supplier matching 'Alfonzo' found. Let's check all suppliers.")
            suppliers = db.query(Supplier).all()
            for s in suppliers:
                print(f"Supplier ID: {s.id}, Name: {s.name}")
